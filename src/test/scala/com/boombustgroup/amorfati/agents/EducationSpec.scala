...
class EducationSpec extends AnyFlatSpec with Matchers:
  ...
  it should "be preserved through copy" in {
    ...
    val copied = hh.copy(monthlyRent = PLN(2000))
  ...
  "LaborMarket.separations" should "retain tertiary workers before primary in automated firms" in {
    ...
    val newFirm  = TestFirmState(
      ...
      PLN(1000000),
      PLN.Zero,
      TechState.Automated(Multiplier(2)),
      Share.decimal(5, 1),
      Multiplier.One,
      Share.decimal(5, 1),
      SectorIdx(0),
    ...
    val hhPrimary    = TestHouseholdState(
      ...
      consumerDebt = PLN.Zero,
      education = 0,
      taskRoutineness = Share.decimal(80, 2),
      wageScar = Share.Zero,
    )
    val hhTertiary   = TestHouseholdState(
      ...
      consumerDebt = PLN.Zero,
      education = 3,
      taskRoutineness = Share.decimal(25, 2),
      wageScar = Share.Zero,
    )
    val hhVocational = TestHouseholdState(
      ...
      Share(0),
      Share.decimal(85, 2),
      HhStatus.Employed(FirmId(0), SectorIdx(0), PLN(5500)),
      Array.empty[HhId],
      bankId = BankId(0),
      equityWealth = PLN.Zero,
      lastSectorIdx = SectorIdx(-1),
      ...
      consumerDebt = PLN.Zero,
      education = 1,
      taskRoutineness = Share.decimal(65, 2),
      wageScar = Share.Zero,
    )
  ...
  it should "use skill as tiebreaker within same education level" in {
    ...
    val hhHighSkill = TestHouseholdState(
      ...
      Share(0),
      Share.decimal(85, 2),
      HhStatus.Employed(FirmId(0), SectorIdx(0), PLN(7000)),
      Array.empty[HhId],
      bankId = BankId(0),
      equityWealth = PLN.Zero,
      lastSectorIdx = SectorIdx(-1),
    ...
    val hhMidSkill  = TestHouseholdState(
      ...
      Share(0),
      Share.decimal(85, 2),
      HhStatus.Employed(FirmId(0), SectorIdx(0), PLN(6500)),
      Array.empty[HhId],
      bankId = BankId(0),
      equityWealth = PLN.Zero,
      lastSectorIdx = SectorIdx(-1),
