...
class CentralBankSpec extends AnyFlatSpec with Matchers:
  ...
  "Nbp.shouldTaperQe" should "be true when inflation exceeds target" in {
    Nbp.shouldTaperQe(Rate(decimal(p.monetary.targetInfl) + BigDecimal("0.01")), Rate(decimal(p.monetary.targetInfl) + BigDecimal("0.005"))) shouldBe true
  }

  it should "be false when expectations remain below target" in {
    Nbp.shouldTaperQe(Rate(decimal(p.monetary.targetInfl) + BigDecimal("0.01")), Rate(decimal(p.monetary.targetInfl) - BigDecimal("0.01"))) shouldBe false
  }
