...
class SectoralMobilitySpec extends AnyFlatSpec with Matchers:
  ...
  it should "decrease monotonically with friction" in {
    for f <- 1 to 10 do
      val low  = SectoralMobility.crossSectorWagePenalty(Share(f * BigDecimal("0.1") - BigDecimal("0.1"))).bd
      val high = SectoralMobility.crossSectorWagePenalty(Share(f * BigDecimal("0.1"))).bd
      high should be <= low
  }
