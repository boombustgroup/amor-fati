...
class BondAuctionSpec extends AnyFlatSpec with Matchers:
  ...
  "foreignDemandShare" should "increase with higher yield spread vs Bund" in {
    val lowSpread  = BondAuction.foreignDemandShare(Rate(0.03), Ratio.Zero)
    val highSpread = BondAuction.foreignDemandShare(Rate(0.08), Ratio.Zero)
    highSpread.toDouble should be > lowSpread.toDouble
  }

  it should "decrease with PLN depreciation (risk-off)" in {
    val stable      = BondAuction.foreignDemandShare(baseYield, Ratio.Zero)
    val depreciated = BondAuction.foreignDemandShare(baseYield, Ratio(0.05))
    depreciated.toDouble should be < stable.toDouble
  }

  it should "increase with PLN appreciation" in {
    val stable      = BondAuction.foreignDemandShare(baseYield, Ratio.Zero)
    val appreciated = BondAuction.foreignDemandShare(baseYield, Ratio(-0.05))
    appreciated.toDouble should be > stable.toDouble
  }

  it should "not exceed maxForeignShare" in {
    val extreme = BondAuction.foreignDemandShare(Rate(0.20), Ratio(-0.10))
    extreme.toDouble should be <= p.fiscal.maxForeignShare.toDouble
  }

  it should "not go below zero" in {
    val extreme = BondAuction.foreignDemandShare(Rate(0.001), Ratio(0.50))
    extreme.toDouble should be >= 0.0
  }

  it should "equal baseForeignShare when spread = 0 and ER stable" in {
    val share = BondAuction.foreignDemandShare(p.fiscal.bundYield, Ratio.Zero)
  ...
  "auction" should "produce bid-to-cover >= 1 under normal conditions" in {
    val result = BondAuction.auction(issuance, bankCap, baseYield, Ratio.Zero)
  ...
  it should "return zero activity for zero issuance" in {
    val result = BondAuction.auction(PLN.Zero, bankCap, baseYield, Ratio.Zero)
  ...
  it should "increase foreign absorption when yield rises" in {
    val lowYield  = BondAuction.auction(issuance, bankCap, Rate(0.03), Ratio.Zero)
    val highYield = BondAuction.auction(issuance, bankCap, Rate(0.08), Ratio.Zero)
  ...
  it should "reduce foreign absorption during PLN depreciation" in {
    val stable = BondAuction.auction(issuance, bankCap, baseYield, Ratio.Zero)
    val crisis = BondAuction.auction(issuance, bankCap, baseYield, Ratio(0.10))
