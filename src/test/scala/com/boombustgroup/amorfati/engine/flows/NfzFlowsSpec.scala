...
class NfzFlowsSpec extends AnyFlatSpec with Matchers:
  ...
  "NfzFlows" should "preserve total wealth at exactly 0L" in {
    val flows    = NfzFlows.emit(NfzFlows.NfzInput.fromDrivers(employed = 80000, wage = PLN(7000), workingAge = 90000, nRetirees = 1000))
  ...
  it should "emit gov subvention only when spending exceeds contributions" in {
    ...
    val deficit = NfzFlows.emit(NfzFlows.NfzInput.fromDrivers(employed = 1000, wage = PLN(5000), workingAge = 5000, nRetirees = 5000))
    deficit.exists(_.mechanism == FlowMechanism.NfzGovSubvention.toInt) shouldBe true

    // Very high wages, tiny population = surplus
    val surplus = NfzFlows.emit(NfzFlows.NfzInput.fromDrivers(employed = 80000, wage = PLN(50000), workingAge = 1000, nRetirees = 10))
  ...
  it should "match old SocialSecurity.nfzStep amounts exactly" in {
    val employed = 80000; val wage = PLN(7000); val workingAge = 90000; val nRetirees = 1000
