...
class IntermediateMarketSpec extends AnyFlatSpec with Matchers:
  ...
  "IntermediateMarket.process" should "produce zero-sum cash adjustments" in {
    ...
    decimal(result.cashAdjustments.sumPln) shouldBe (decimal(PLN.Zero) +- decimal(PLN("1.0")))
  ...
  it should "exclude bankrupt firms from buying and selling" in {
    ...
    decimal(firms.zip(result.cashAdjustments).filter((firm, _) => Firm.isAlive(firm)).map(_._2).sumPln) shouldBe (decimal(PLN.Zero) +- decimal(PLN("1.0")))
  }
