...
class EquityMarketPropertySpec extends AnyFlatSpec with Matchers with ScalaCheckPropertyChecks:
  ...
  "EquityMarket.computeDividends" should "have non-negative outputs for positive inputs" in
    forAll(Gen.choose(0.0, 1e10), Gen.choose(0.01, 0.15), Gen.choose(1e6, 1e13), genFraction) { (profits, divYield, mcap, foreignShare) =>
  ...
  it should "have domestic + foreign + tax ≈ total dividends" in
    forAll(Gen.choose(0.0, 1e10), Gen.choose(0.01, 0.15), Gen.choose(1e6, 1e13), Gen.choose(0.0, 1.0)) { (profits, divYield, mcap, foreignShare) =>
