...
/** Mechanism toggles for 49 model features.
  ...
  * SFC-ABM engine. Defaults match current `object Config` env-var defaults for
  * PR 1 backward compatibility. In PR 2 all defaults flip to `true` (full
  * Polish economy baseline). Flags are used exclusively for counterfactual
  * experiments — not for reproducing legacy behavior.
  *
  * Grouped by domain:
  *
