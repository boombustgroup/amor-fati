...
object Simulation:
  ...
  def step(state: SimState, masterSeed: Long, month: Int)(using SimParams): StepResult =
    ...
    val migRng                         = new Random(StepSeeds.derive(masterSeed, month, StepSeeds.RegionalMigration))
    ...
    val s10                            = S10.run(S10.Input(w, firms, households, s1, s2, s3, s4, s5, s6, s7, s8, s9), waRng, migRng)
