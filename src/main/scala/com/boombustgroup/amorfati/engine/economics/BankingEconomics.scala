...
object BankingEconomics:
  ...
  def runStep(in: StepInput)(using p: SimParams): StepOutput =
    ...
    val bfgLevy              =
      if true then Banking.computeBfgLevy(in.banks).total
      else PLN.Zero
  ...
  private def computeGovAndJst(in: StepInput)(using p: SimParams): GovJstResult =
    ...
    val socialTransferSpend =
      if true then in.s3.hhAgg.totalSocialTransfers
      else PLN.Zero
  ...
  private def updateSingleBank(
    ...
    val bankBfgLevy               =
      if true && !b.failed then b.deposits * p.banking.bfgLevyRate.monthly
      else PLN.Zero
